  ...
  return (
    <div className={`border rounded-md overflow-hidden ${isExpanded?"border-white/15 bg-[#15171e]":"border-white/8 bg-[#0e0f12]"}`}>
          ...
          <div className="border border-white/8 rounded-lg overflow-hidden">
            ...
            {showVideoSelector && (
              <div className="border-t border-white/8 bg-[#0e0f12]">
                ...
                {/* Option 3: Select from history — opens modal with thumbnails */}
                {completedVideos.length > 0 && (
                  <div className="border-t border-white/8 px-2 py-2">
                    <button
                      onClick={e=>{e.stopPropagation();setShowVideoSelector(false);setShowVideoModal(true);}}
                      className="w-full flex items-center justify-center gap-1.5 text-[9px] py-1.5 bg-cyan-400/5 border border-cyan-400/20 text-cyan-400/70 hover:text-cyan-400 hover:bg-cyan-400/10 hover:border-cyan-400/40 rounded transition-all"
                    >
                      <Video className="w-3 h-3"/>
                      Seleccionar del historial ({completedVideos.length} videos)
                    </button>
                  </div>
                )}
              </div>
      ...
      {showVideoModal && (
        ...
        >
          <div
            className="bg-[#0e0f12] border border-white/15 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl"
            onClick={e=>e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/8">
              <div>
                <h3 className="text-sm font-black text-white">Seleccionar video de referencia</h3>
                <p className="text-[10px] text-gray-500 mt-0.5">{completedVideos.length} videos generados disponibles</p>
              </div>
              <button onClick={()=>setShowVideoModal(false)} className="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-white/10 transition-colors">
                <X className="w-4 h-4"/>
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {completedVideos.map(task=>{
                  ...
                  return (
                    <button
                      key={task.id}
                      onClick={()=>{ onUpdateField("video_url", isSelected ? "" : videoUrl); setShowVideoModal(false); }}
                      className={`relative rounded-xl overflow-hidden border-2 transition-all group ${
                        isSelected ? "border-cyan-400 ring-2 ring-cyan-400/30" : "border-white/10 hover:border-cyan-400/50"
                      }`}
                    >
                      {/* Thumbnail */}
                      <div className="aspect-video bg-black relative">
                        ...
                        />
                        {isSelected && (
                          <div className="absolute top-1.5 right-1.5 bg-cyan-400 text-black rounded-full p-0.5">
                            <Check className="w-3 h-3"/>
                          </div>
                        )}
                      </div>
                      {/* Label */}
                      <div className="px-2 py-1.5 bg-[#0e0f12] text-left">
                        <p className="text-[10px] text-gray-300 font-medium truncate">
                          {task.sceneTitle ? `${task.sceneTitle} · Shot ${task.clipNumber}` : `Shot ${task.clipNumber || task.id.slice(0,8)}`}
                        </p>
                        <p className="text-[9px] text-gray-600 mt-0.5">{isSelected ? "✓ Seleccionado" : "Usar como referencia"}</p>
                      </div>
                    </button>
                  );
                })}
            ...
            {/* Footer */}
            {clip.video_url && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-white/8 bg-cyan-400/5">
                <span className="text-[10px] text-cyan-400">Video de referencia seleccionado ✓</span>
                <button
                  onClick={()=>{ onUpdateField("video_url",""); setShowVideoModal(false); }}
                  className="text-[9px] text-red-400 hover:text-red-300 px-2 py-1 bg-red-400/10 rounded hover:bg-red-400/20 transition-colors"
                >
                  Quitar referencia
                </button>
              </div>
